      ...
      >
        ...
        >
          ...
          >
            ...
            <!-- <v-icon @click="downloadImage(post.imageName)">download</v-icon> -->
            <v-card-text @click.stop="downloadImage(post.imageName)">{{post.title}}</v-card-text>
          </v-card>
        </v-flex>
      </v-layout>
...
export default {
  ...
  computed: {
    ...mapGetters(["user", "userFavorites", "userPosts", "downloadImagePath"])
  },
  watch: {
  ...
  methods: {
    downloadImage(imageName) {
      ...
      if (this.downloadImagePath) {
        console.log(this.downloadImagePath);
        return (this.downloadDialog = true);
      }
    },
    goToPost(id) {
    ...
    loadPost(
      { _id, title, categories, description, imageBase64 },
      editPostDialog = true
    ) {
